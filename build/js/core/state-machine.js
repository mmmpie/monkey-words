(function () {
    ...
    feature().stateMachine = function () {
        ...
        _public.setInitialState = function (name) {
            ...
            if (!initialState) {
                throw 'cant set final on a non existant state';
            }
            states = newStates;
            return _public;
        };
        ...
        _public.step = function (event) {
            ...
            _log.info(event);
            if (!running) {
                throw 'state machine cannot be advanced, it is not running';
            }
            ...
            var areAllFsmsTerminal = _.invoke(subFsms, 'isTerminal').reduce(function (allTerminal, terminal) {
                return allTerminal && terminal;
            }, true);
            if (areAllFsmsTerminal) {
                ...
                if (validTransition) {
                    transition(states[validTransition]);
                }
                else {
            ...
            var areAnyFsmsInError = _.invoke(subFsms, 'isError').reduce(function (anyError, error) {
        ...
        _public.isTerminal = function () {
            if (currentState) {
                return currentState.terminal === true;
            }
            return false;
        };
